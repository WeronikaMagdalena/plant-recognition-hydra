...
def find_best_split(X, y, n_bins=8, max_evals=5000, rng=None):
    ...
    feature_indices = rng.choice(n_features, n_sub_features, replace=False)
    evals = 0
    for feature_index in feature_indices:
        col = X[:, feature_index]
        thresholds = np.unique(np.percentile(col, np.linspace(0, 100, n_bins)))
        for threshold in thresholds:
            ...
            gr = gain_ratio(y, y_left, y_right)
